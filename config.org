...
#+begin_src emacs-lisp
  ...
  (defconst zettelkasten-dir "/Users/florian/Nextcloud/proj/Zettelkasten")
  (defconst emacs-dir (file-name-directory (or load-file-name (buffer-file-name))))
#+end_src

* Basic Editor Behaviour

This is a hack to make eglot-jl work:
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (when ismac
    ...
    (use-package exec-path-from-shell
      ...
      (dolist (var '("PATH" "MANPATH" "LANG" "LC_ALL" "LC_CTYPE"))
...
** Custom Link Types
Hookmark:
#+name: org-custom-links
#+begin_src emacs-lisp :tangle no
  (defun my-org-open-url-via-os (url)
    "Open URL via the OS default handler (so custom schemes like hook:// work)."
    (pcase system-type
      ('darwin     (start-process "open-url" nil "open" url))
      ('gnu/linux  (start-process "open-url" nil "xdg-open" url))
      ('windows-nt (w32-shell-execute "open" url))
      (_           (browse-url url))))

  (org-link-set-parameters
   "hook"
   :follow (lambda (path _arg)
             (my-org-open-url-via-os (concat "hook:" path))))

  (org-link-set-parameters
   "busycalevent"
   :follow (lambda (path _arg)
             (my-org-open-url-via-os (concat "busycalevent:" path))))
#+end_src
** Org-modern
...
** GTD
Tags are used as contexts:
#+name: org-gtd-contexts
#+begin_src emacs-lisp :tangle no
...
Keywords represent action types:
#+name: org-gtd-actions
#+begin_src emacs-lisp :tangle no
  (setq org-todo-keywords
        '((sequence "TODO(t)" ;; Next action
                    "STARTED(s!)" ;; Started
                    "WAITING(w@/!)" ;; Waiting for (list). Not sure wheter promting for explaination is needed.
                    "APPT(a!)" ;; Calendar item
                    "|"
                    "DONE(d!)" ;; Successful completion
                    "CANCELED(c@)"))) ;; Could not be completed (promt for explaination)
#+end_src

Create an ID for each captured item:
#+name: org-gtd-capture-id
#+begin_src emacs-lisp :tangle no
#+end_src

Keep agenda files minimal:
#+name: org-gtd-gtd-files
#+begin_src emacs-lisp :tangle no
  (setq org-agenda-files (list "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093017==99a--gtd-inbox.org"
  			     "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093024==99b--gtd-actions.org"
  			     "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093053==99e--gtd-areas.org"
  			     ))
#+end_src

Minimal set of capture templates
#+name: org-gtd-gtd-capture
#+begin_src emacs-lisp :tangle no
  (setq org-capture-templates
        '(("t" "Inbox" entry
           (file "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093017==99a--gtd-inbox.org")
           "* TODO %?\n:PROPERTIES:\n:CREATED: %U\n:ID: %(org-id-new)\n:END:\n"
           :empty-lines 0)
  	("p" "Project" entry
           (file "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093035==99c--gtd-projects.org")
           "* %?\n:PROPERTIES:\n:CREATED: %U\n:ID: %(org-id-new)\n:END:\n"
           :empty-lines 0)
  	  	("P" "Area of Focus" entry
           (file "/Users/florian/Nextcloud/proj/Zettelkasten/20260101T093053==99e--gtd-areas.org")
           "* %?\n:PROPERTIES:\n:CREATED: %U\n:ID: %(org-id-new)\n:END:\n"
           :empty-lines 0)
  	))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq org-agenda-custom-commands
      '(("g" "Grouped TODOs + Scheduled next 14 days"
         ((alltodo ""
          ...
          (agenda ""
                  ((org-agenda-span 14)
                   ...
                   (org-agenda-overriding-header "Scheduled (next 14 days)")
...
#+begin_src emacs-lisp
  ...
  ;; Tags should appear directly after text:
...
Taken from meow, to quit a window:
#+begin_src emacs-lisp
(defun cst/window-quit ()
...
#+begin_src emacs-lisp
  (defun fst/other-window-backwards (&optional n)
    "Select the previous window. With prefix arg N, move backward N windows."
    (interactive "p")
    (other-window (- n)))
#+end_src
...
#+begin_src emacs-lisp
  (when (not iswindows)
    (use-package vterm
      :straight t
      :config
      (define-key vterm-mode-map (kbd "C-;") #'ryo-modal-mode)
      )
    )
#+end_src
* Denote
...
Base package:
  #+begin_src emacs-lisp
    (use-package denote
      ...
      (setq denote-directory (expand-file-name zettelkasten-dir))
    ...
    (ryo-modal-keys
     ("n"
      (("f" consult-notes :name "Note find")
       ("s" denote-sequence-new-sibling-of-current :name "New Sibling")
       ("c" denote-sequence-new-child-of-current :name "New Child")
       ("l" denote-link :name "New Link")
       ))
     )
...
#+begin_src emacs-lisp
  (use-package auctex
    ...
    (setq TeX-auto-local ".build")
    (setq TeX-parse-self t)
    (setq-default TeX-master nil)

    ;; (ryo-modal-major-mode-keys
    ;;  '(LaTeX-mode latex-mode)
    ;;  ;; ("c c" TeX-command-default)
    ;;  ("c v" TeX-view)
    ;;  )
    ...
    (defun latex-compilation-commands-hook ()
      (add-to-list 'TeX-command-list
  		 '("LaTeXmk (rcfile)" "latexmk %s" TeX-run-TeX nil t
                     :help "Run latexmk using .latexmkrc on file"))
      (add-to-list 'TeX-command-list
  		 '("LaTeXmk (PDF)" "latexmk -pdf -pdflatex='pdflatex -synctex=1' -norc %s" TeX-run-TeX nil t
                     :help "Run latexmk with pdflatex on file."))
      (add-to-list 'TeX-command-list
  		 '("LaTeXmk (Clean)" "latexmk -C %s" TeX-run-TeX nil t
                     :help "Run latexmk Clean."))
      (add-to-list 'TeX-command-list
  		 '("LaTeXmk (Lua)" "latexmk -pdf -pdflatex='lualatex -synctex=1' -norc %s" TeX-run-TeX nil t
                     :help "Run latexmk with lualatex on file."))
      (add-to-list 'TeX-command-list
  		 '("Make" "make %s.pdf" TeX-run-TeX nil t
                     :help "Run Makefile in master directory."))
      (add-to-list 'TeX-command-list
  		 '("Make (all)" "make all" TeX-run-TeX nil t
                     :help "Run Makefile in master directory."))
      (add-to-list 'TeX-command-list
  		 '("Make Clean" "make clean" TeX-run-TeX nil t
                     :help "Run Makefile in master directory.")))


    (add-hook 'LaTeX-mode-hook 'latex-compilation-commands-hook)
...
** Local Map
Local keymap for ess-r-mode:
#+name: local-map-ess-r
#+begin_src emacs-lisp :noweb no
  (with-eval-after-load 'ess-r-mode
  (ryo-modal-major-mode-keys
   'ess-r-mode
   ("c b" ess-eval-buffer)
   ("c x" fst/ess-restart-r)
   ("c l" ess-eval-region-or-line-and-step)
   )
    )
#+end_src
** ESS Package Setup
...
#+begin_src emacs-lisp
  ...
  (use-package julia-ts-mode
    ...
    (defvar cst/julia-repl-window nil)
    (defun cst/julia-repl-start ()
    "Splits window and opens julia repl"
    (interactive)
    (let ((script-buffer (current-buffer))
  	(inferior-buffer (julia-repl-inferior-buffer))
          (inferior-window (split-window (selected-window) (* (display-pixel-height) 0.70) 'below t)))
      (with-current-buffer inferior-buffer (setq julia-repl--script-buffer script-buffer))
      (with-selected-window inferior-window (switch-to-buffer inferior-buffer))
      (set 'cst/julia-repl-window inferior-window)
      )
    )

  (defun cst/julia-repl-restart ()
    "Restarts julia repl"
    (interactive)
    (let ((script-buffer (current-buffer))
          (inferior-new nil)
      ...
      (with-selected-window cst/julia-repl-window
        (kill-buffer inferior-buffer)
        (setq inferior-new (julia-repl-inferior-buffer))
        (switch-to-buffer inferior-new)
        )
      )
    )

    (defun cst/julia-repl-send-main ()
    "..."
    (interactive)
    (julia-repl--send-string (concat "include(" "\"" (concat (expand-file-name (project-root (project-current t))) "src/main.jl") "\")")))

    ;; Keymap:
      (ryo-modal-major-mode-keys
       'julia-ts-mode
       ("c b" julia-repl-send-buffer)
       ("c x" cst/julia-repl-restart)
       ("c m" cst/julia-repl-send-main)
       ("c l" julia-repl-send-region-or-line)
       )
      )
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package julia-repl
    ...
    ;;(define-key julia-repl-mode-map (kbd "C-;") #'ryo-modal-mode)
    )
#+end_src
* Common Lisp
#+begin_src emacs-lisp
...
Consult only files:
#+begin_src emacs-lisp
(defun fst/consult-find-only-files ()
  "Like `consult-find', but list only files (no directories)."
  (interactive)
  (require 'consult)
  (let ((consult-find-args
         (if (listp consult-find-args)
             ;; In case you're on a variant where args are a list.
             '("find" "." "-type" "f" "-not" "(" "-wholename" "*/.*" "-prune" ")")
           ;; Default consult uses a string.
           "find . -type f -not ( -wholename */.* -prune )")))
